...
Chat.prototype.make_initial = function(){
    var self = this;
    return function(seqno){
	console.log("initial chat");
	var myVar = setInterval(function(){self.heartbeat();},60000);
	if(self.roster.indexOf(screen_name) == -1){
	self.roster.push(screen_name);
	document.getElementById('menu').innerHTML = '<p><b>Member</b></p>';
	document.getElementById('menu').innerHTML += '<ul><li>'+self.roster[0]+'</li></ul>';
	var d = new Date();
	var t = d.getTime();
	self.msgcache.push({seqno:sync.usrseq,msgtype:"join",msg:"xxx",time:t});
	while (self.msgcache.length>self.maxmsgcachelength)
	self.msgcache.shift();
	}
    }
};

Chat.prototype.make_sendInterest = function(){
    var self = this;
    return function(content){
	    ...
	    if(name_t!=screen_name){//won't fetch the data from user that has the same name as me
	        ...
	        ndn.expressInterest(n, template, self.make_onData(), self.chatTimeout);
...
Chat.prototype.make_onInterest = function(){
    var self = this
    return function(inst){
        ...
        //console.log("seq"+seq);
        for(var i = self.msgcache.length-1;i>=0;i--){
	//console.log("msgseq:"+msgcache[i].seqno);
            if(self.msgcache[i].seqno ==seq){
                content = {msg:self.msgcache[i].msg,type:self.msgcache[i].msgtype,time:self.msgcache[i].time};
...
Chat.prototype.make_onData = function(){
    var self = this;
    return function(inst,co){
	    ...
	    objDiv.scrollTop = objDiv.scrollHeight;
            ...
            var n = self.roster.indexOf(name);
	    if(n!=-1 && name!=screen_name){
                self.roster.splice(n,1);
	        document.getElementById('menu').innerHTML = '<p><b>Member</b></p><ul>';
	        for(var i = 0;i<self.roster.length;i++){
	            document.getElementById('menu').innerHTML += '<li>'+self.roster[i]+'</li>';
	        }
...
Chat.prototype.heartbeat=function(){
	...
	var template = new Interest();
	template.interestLifetime = 10000;
	ndn.expressInterest(n, template, sync.make_onData(this.make_sendInterest(),this.make_initial()), sync.make_syncTimeout(this.make_sendInterest(),this.make_initial()));                
